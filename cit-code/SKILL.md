...
Write config to `~/.claude/CLAUDE.md` (see CIT-Config for the block format).
...
Users can override at any time: "treat this as Major" is honored without argument.
...
Files that directly import or call the changed component. Always traced. Always shown.
...
Config files, test suites, CI/CD pipelines, environment variables, external API contracts, database migrations. Always surfaced as an explicit human handoff checklist.

---

## File Request Protocol

When the trace reaches a reference Claude cannot resolve, it does a partial trace first, identifies exactly where the trace goes blind, then requests the specific files needed:
...
A map with marked unknowns is more trustworthy than one that silently omits them.
...
### CIT_SHOW: OFF:
All trace sections run internally. Implementation is written after the trace completes. If conflicts are found, they are surfaced. Conflicts are never hidden regardless of the visibility setting.
