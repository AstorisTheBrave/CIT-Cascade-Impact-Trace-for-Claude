...
If a `[CIT-CONFIG]` block is found, load it silently and confirm:
...
- Note: Tree handles complex webs better. Domino is more visual but auto-switches to Tree beyond 8 nodes.
...
- For "Finish current": Safer. The action proceeds with full cascade awareness. Takes slightly longer.
- For "Stop immediately": Faster, but the current action proceeds without a complete trace. Use with caution.

After all three answers are collected, write the config block and confirm:
...
Mark blind spots explicitly and never skip silently.
...
No silent pivots. Acknowledge every revision to prior output.
[/CIT-CORE-PHILOSOPHY]
[/CIT-CONFIG]
```

**claude.ai** - write the same block to Claude Memory with the tag `[CIT-CONFIG]` and inform the user:
...
| `cit settings` | Show current config and offer to change any setting |
...
If the version is mismatched or the block is missing, prompt the user to run `cit settings` to update.
