...
CIT is a set of Claude skills that enforce one discipline: **before you change anything, map everything it touches — all the way to the last domino.**
...
|---|---|---|
| **CIT-Code** | Claude Code | Cascde traces across files, modules, and architecture before any code change |
| **CIT-Chat** | claude.ai | Tracks claims, assumptions, and recommendations across a conversation — no silent pivots |
...
**CIT-Config** is the optional shared config that all three skills pull from. Install it once, all three skills stay in sync.
...
- `CIT_SHOW: OFF` — Claude runs the trace silently, acts with full awareness
...
This gives you shared config across all skills — set up once, all three stay in sync.
...
### First-time setup
On first run, CIT asks you three questions via an interactive prompt:
1. Show traces visually or run silently?
2. Tree view or Domino chain?
3. If you type `cit off` mid-task — complete the current trace first, or stop immediately?
